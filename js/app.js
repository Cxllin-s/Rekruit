...
<<<<<<< codex/deploy-recruit-hr-site-with-basic-functionality-ootrbk
const USER_CREDENTIALS_KEY = "rekruit_user_credentials_v1";
=======
>>>>>>> main
...
function findCredentialByEmail(email) {
  ...
  return loadCredentials().find((u) => (u.email || "").toLowerCase() === key);
}
...
(function signupPage() {
  ...
  form.addEventListener("submit", (e) => {
    ...
    credentials.push({ id: uid(), name, email, password, role, createdAt: nowIso() });
    saveCredentials(credentials);

    const data = loadData();
    if (!data.users.some((u) => (u.email || "").toLowerCase() === email)) {
      data.users.unshift({ id: uid(), name, email, role });
      addActivity(data, `User signed up: ${name}`);
      saveData(data);
    }

    showFlash("Account created. Redirecting to login.", "ok");
